...
> 詳細（SOLID全体・命名例・コード臭表・リファクタリング手順など）は `member-materials/01_講義資料.md` を読んでおくと理解が深まります。
...
L・I・Dの詳細は `member-materials/01_講義資料.md` のに記載しています。
...
`member-materials/02_演習_問題コード.py` を開いてください
...
`member-materials/04_変更影響チェックリスト.md` を開き、**Step 1・2のみ**を記入しながら進めます（Step 3〜5は各自でトライしてみください）。
